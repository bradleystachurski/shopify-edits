(function() {
  ...
  jQuery(function($) {
    ...
    searchAndAccount = function() {
      $('.searchbar-open').click(function() {
        // $(this).closest('.menu').fadeOut(100, function() {
          $('.main-header .searchbar-container').css({'display':'block'});
          $('.main-header .searchbar-container').fadeIn(200);
          // return 
          $('.main-header .searchbar-container .search-box').focus();
        // });
        return false;
      });
      ...
      HEADER.find(".main-menu .widescreen .dropdown > a").click(function() {
        ...
        if (dropdown.hasClass("active")) {
          ...
          dropdown_panel.slideDown(400, function() {
            dropdown_panel.css("display", 'block');
    ...
    mobileMenu = function() {
      ...
      mobile_menu.find('.nav-item').has('.sub-nav').find('> .nav-item-link').append('<span aria-hidden="true" class="glyph plus"></span><span aria-hidden="true" class="glyph minus" style="display: none;"></span>');
...
//Bold: Ensure the cart's line items have been updated with their option information
  if(typeof(BOLD) === 'object' && BOLD.helpers && typeof(BOLD.helpers.cleanCart) === 'function' )
    cart = BOLD.helpers.cleanCart(cart);
        ...
        cart_item.recurring = cart.items[0].formatted_recurring_desc;
        ...
        if (cart_item.recurring == undefined) {
            cart_item.recurring = "";
        }
        ...
        new_cart_row += '<h2><a href="' + cart_item.url + '">' + cart_item.title + cart_item.recurring + '</a></h2>';
        new_cart_row += '</td>';
        ...
        $('.recently-added').removeClass('hide');
    ...
    if (PAGE.hasClass('template-index')) {
      ...
      if (!$('.product-slider').hasClass('disabled')) {
